...
- 背景纹理先进入 renderer 的离屏 framebuffer。固定的歌词背板是 `glass-shape` 元素，使用上游的连续曲率 SDF、局部 FBO、折射、可分离模糊、高光和阴影 pass。
- 歌词是单独缓存的前景纹理，按播放进度由临界阻尼弹簧滚动，并使用上游 `clipRect` 裁剪在玻璃容器内。仅提交当前行前后四行，避免长歌词将屏幕外内容带入渲染循环。
