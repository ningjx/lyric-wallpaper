...
在 Wallpaper Engine 里右键壁纸 → **自定义**，只有一个属性：

| 参数 | 键 | 默认 | 说明 |
|------|----|------|------|
| 显示壁纸调参 | `showcontrols` | 关闭 | 打开后在壁纸右下角显示液态玻璃参数面板；面板关闭后不会遮挡壁纸。 |

液态玻璃的字体、歌词间距、背板内边距、空间层次和材质等参数暂时保留在壁纸内的调参面板中，后续再按需要加入 Wallpaper Engine 属性栏。
