...
**双循环解耦**（动画不依赖 API 轮询）：
- API：1 秒轮询 `/query`，只做"校准"
- 渲染：rAF 驱动 `SyncClock.now()`，本地单调时钟平滑推进
- 暂停 → 时钟停住；恢复 → 继续；拖进度条大跳变 → 直接跳转

**渲染器**：固定 7 行 DOM 池，每个元素绑定一个绝对歌词行索引；当前行索引变化时整组平移一格（CSS transition 平滑滚动），滑出窗口的行回收后从另一侧滑入。当前行 `font-size: 46px` 居中，上下行逐级缩小（33/26/22px）并降低透明度、加模糊。
